...
class AddCustomerTableViewController: UITableViewController {
    ...
    @IBAction func saveButtonTapped(_ sender: UIBarButtonItem) {
        ...
        if let ref = ref {
            ...
            ref.child(Table.customers.rawValue).observe(.value) { cust_snapshot in
                self.customerCount = Int(cust_snapshot.childrenCount)
                if cust_snapshot.childrenCount > 0  {
                    ref.child(Table.tables.rawValue).observe(.value) { table_snapshot in
                        self.tableCount = Int(table_snapshot.childrenCount)
                        if cust_snapshot.childrenCount <= table_snapshot.childrenCount && self.tableID != nil && self.tableReference != nil {
